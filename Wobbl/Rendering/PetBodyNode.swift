...
final class PetBodyNode: SKShapeNode {
    ...
    func setup() {
        ...
        fillColor = ColorPalette.normalBody.withAlphaComponent(0.25)
        strokeColor = ColorPalette.normalStroke.withAlphaComponent(0.5)
        lineWidth = 2.0
        glowWidth = 0
        isAntialiased = true
    }
    ...
    func transitionColor(to color: SKColor, duration: TimeInterval = 0.5) {
        let targetFill = color.withAlphaComponent(0.25)
        let targetStroke = color.withAlphaComponent(0.5)
